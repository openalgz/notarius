...
namespace slx
{
   namespace chrono
   {
      ...
      std::string write_ave_duration_message(std::chrono::steady_clock::duration duration)
      {
         ...
         if (nanoseconds > 0) {
            return std::format(
               "{:02}<span style = \"color:#B22222\">m</span> {:02}<span style = \"color:#B22222\">s</span> {:03}<span "
               "style = \"color:#B22222\">ms</span> {:09}<span style = \"color:#B22222\">ns</span>",
               minutes, seconds, milliseconds, nanoseconds);
         }
         return std::format(
            "{:02}<span style = \"color:#B22222\">m</span> {:02}<span style = \"color:#B22222\">s</span> {:03}<span "
            "style = \"color:#B22222\">ms</span>",
